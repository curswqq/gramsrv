...
async function sendInvoice(ctx, product, targetUserID, language, extra = "") {
  ...
  await ctx.api.sendInvoice(ctx.chat.id, localized.title, localized.description, buildPayload(product.code, targetUserID, extra), "XTR", [{ label: localized.title, amount: product.starsPrice }]);
}
...
export function createBot({ config, db, gramsrv }) {
  ...
  async function fulfill(product, recipientID, buyer, chatID, chargeID, extra = "") {
    ...
    db.addSale({ product: product.code, title: productView.title, starsPrice: product.starsPrice, recipientID, buyerID: buyer.id, buyerName: userName(buyer), chargeID });
  ...
  bot.on("message:text", async (ctx) => {
    ...
    try {
      ...
      if (pending.kind === "admin_refund") {
        const chargeID = input.trim(); const sale = db.saleByCharge(chargeID);
        ...
        await bot.api.refundStarPayment(sale.buyer_id, chargeID); db.markRefunded(chargeID, sale.buyer_id); db.clearPending(ctx.from.id);
        await bot.api.sendMessage(sale.buyer_id, tr(sale.buyer_id, "paymentRefunded", { charge: escapeHTML(chargeID) }), { parse_mode: "HTML" }).catch(() => {});
