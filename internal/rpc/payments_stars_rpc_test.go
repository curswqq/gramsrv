...
func TestOnPaymentsGetStarsStatusAndTransactionsIOSDisplay(t *testing.T) {
	const userID int64 = 1000000001
	const counterpartyUserID int64 = 1000000002
	const counterpartyChannelID int64 = 2000000001

	svc := appstars.NewService(memory.NewStarsStore(), appstars.WithStartingGrant(500))
	ctx := WithUserID(context.Background(), userID)

	// Topup (Fragment)
	if _, err := svc.Credit(ctx, userID, 100, domain.StarsReasonTopup, domain.Peer{}, "", ""); err != nil {
		t.Fatalf("credit topup: %v", err)
	}
	// Gift from user
	if _, err := svc.Credit(ctx, userID, 50, domain.StarsReasonGift, domain.Peer{Type: domain.PeerTypeUser, ID: counterpartyUserID}, "", ""); err != nil {
		t.Fatalf("credit gift: %v", err)
	}
	// Reaction to channel
	if _, err := svc.Debit(ctx, userID, 25, domain.StarsReasonReaction, domain.Peer{Type: domain.PeerTypeChannel, ID: counterpartyChannelID}, "", ""); err != nil {
		t.Fatalf("debit reaction: %v", err)
	}

	channelView := domain.ChannelView{
		Channel: domain.Channel{ID: counterpartyChannelID, AccessHash: 12345, Title: "Channel Name"},
		Self:    domain.ChannelMember{ChannelID: counterpartyChannelID, UserID: userID, Status: domain.ChannelMemberActive},
	}
	r := New(Config{}, Deps{
		Stars:    svc,
		Channels: &channelLedgerChannels{view: channelView},
	}, zaptest.NewLogger(t), clock.System)

	// 1. Initial status call (used by iOS on opening the Stars screen)
	status, err := r.onPaymentsGetStarsStatus(ctx, &tg.PaymentsGetStarsStatusRequest{Peer: &tg.InputPeerSelf{}})
	if err != nil {
		t.Fatalf("onPaymentsGetStarsStatus: %v", err)
	}
	history, ok := status.GetHistory()
	if !ok || len(history) != 4 {
		t.Fatalf("initial history length = %d ok=%v, want 4", len(history), ok)
	}
	// Verify non-empty titles for iOS
	for _, txn := range history {
		title, hasTitle := txn.GetTitle()
		if !hasTitle || title == "" {
			t.Fatalf("txn %s has missing title for iOS", txn.ID)
		}
	}
	if len(status.Chats) == 0 {
		t.Fatal("status.Chats must not be empty when transaction involves a channel")
	}

	// 2. Transactions call
	txnsStatus, err := r.onPaymentsGetStarsTransactions(ctx, &tg.PaymentsGetStarsTransactionsRequest{
		Peer:  &tg.InputPeerSelf{},
		Limit: 20,
	})
	if err != nil {
		t.Fatalf("onPaymentsGetStarsTransactions: %v", err)
	}
	if len(txnsStatus.Chats) == 0 {
		t.Fatal("txnsStatus.Chats must contain the counterparty channel for iOS")
	}
}
