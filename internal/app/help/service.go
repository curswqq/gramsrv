...
const tdesktopBusinessPromoAppConfig = `,"business_promo_order":["business_location","business_hours","quick_replies","greeting_message","away_message","business_links","business_intro","business_bots","emoji_status","folder_tags","stories"]`
...
func defaultAppConfigJSONWithPremiumBot(mapboxToken, premiumBotUsername string) []byte {
	...
	if mapboxToken == "" {
		return []byte(baseConfig + tdesktopNoForwardsAppConfig + tdesktopBusinessPromoAppConfig + tdesktopPaidMessagesAppConfig + premiumFragment + androidInvoiceBilling + `}`)
	}
	...
	return []byte(baseConfig + tdesktopNoForwardsAppConfig + tdesktopBusinessPromoAppConfig + tdesktopPaidMessagesAppConfig + premiumFragment + androidInvoiceBilling + `,"tdesktop_config_map":{"maps":` + tokenJSON + `,"geo":` + tokenJSON + `,"bmaps":` + tokenJSON + `,"bgeo":` + tokenJSON + `}}`)
}
